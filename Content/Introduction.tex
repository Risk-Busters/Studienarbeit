...
To address this problem, the authors of this paper employ gamification techniques in the development of a project risk management tool and aim at influencing the motivation of project managers and their teams to counter negligence in the course of a project’s lifetime.
...
During this paper, different steps are taken into account to establish a good foundation to work on and implement a \ac{MVP}:
\begin{itemize}
	\item Research needed theoretical background knowledge (\ref{ch:theorie})
	\item A concept including a survey with potential users, a clickable prototype, use cases and requirements (\ref{ch:domain})
	\item The implementation of the \acs{MVP} itself with a description what is still missing  (\ref{ch:implementation})
\end{itemize}

In the end, the course of this paper is discussed (\ref{ch:discussionA}) and an outlook (\ref{ch:conclusionoutlookB}) is given.
