...
\begin{itemize}
	...
	\item If the visitors input fulfils the criteria of a username, email and password the account is created on the server and a conformation e-mail is send to the given address.
\end{itemize}
...
\begin{itemize}
	\vspace{-3mm}
	\setlength\itemsep{-1em}
	\item If a visitor wants to log into an existing user account or if they attempt to delete an account, the user account's credentials have to be submitted (e-mail and password for login, password only for deletion).
